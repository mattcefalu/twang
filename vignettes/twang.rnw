...
By default, the \texttt{bal.table()} function uses the value of \texttt{estimand} set with the \texttt{ps()} function call.  For example, in the analysis we set \texttt{estimand =  "ATT"} when calling \texttt{ps()} to estimate the propensity scores and the resulting \texttt{ps.object}, \texttt{ps.lalonde}, contains an element ``estimand'' which takes the value \texttt{ "ATT"}.  The function \texttt{bal.table()} checks this value and automatically uses ATT weights when checking balance and comparing the distributions of pre-treatment variables for the
...
\texttt{bal.table()} returns information on the pretreatment covariates before and after weighting. The object is a list with named components, one for an unweighted analysis (named
\texttt{unw}) and one for each \texttt{stop.method} specified, here \texttt{es.mean} and \texttt{ks.max}. McCaffrey et al (2004) essentially used \texttt{es.mean} for the analyses, but
our more recent work has sometimes used \texttt{ks.max}. See McCaffrey et al. (2013) for a more detailed description of these choices.
