bal.table.mediation <- function(x, ...) {
  ...
  balance_nie_1 <- bal.table(dx.wts(nie_1_wts, data = data, vars = x$mediator_names, 
      treat.var = "A", x.as.weights = TRUE, estimand = "ATT"))

  balance_nie_1 <- do.call(rbind, balance_nie_1)
  balance_nie_1["model"] = "NIE_1"
  rows <- rownames(balance_nie_1)
  balance_nie_1 <- balance_nie_1[rows[grep(paste(x$mediator_names, 
      collapse = "|"), rows)], ]
  ...
  balance_nie_0 <- bal.table(dx.wts(nie_0_wts, data = data, vars = x$mediator_names, 
      treat.var = "A", x.as.weights = TRUE, estimand = "ATT"))
  balance_nie_0 <- do.call(rbind, balance_nie_0)
  balance_nie_0["model"] = "NIE_0"
  rows <- rownames(balance_nie_0)
  balance_nie_0 <- balance_nie_0[rows[grep(paste(x$mediator_names, 
      collapse = "|"), rows)], ]

  return(list(balance_a = balance_a, balance_m0 = balance_m0, balance_m1 = balance_m1, 
        balance_nie_1 = balance_nie_1, balance_nie_0 = balance_nie_0))
