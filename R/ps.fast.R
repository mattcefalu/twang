...
ps.fast <- function(formula ,
                    ...
                    #n.grid.ks = 25,
                    #n.grid.es = NULL,
                    moderator = NULL, #String with moderator name
                    ...){
  ...
  # setup balance data
  bal.data = gen.bal.data(data=data , var.names=var.names, mod.name = moderator)
  ...
  if ( any(grepl("es.",stop.method.names)) ){
    ...
    std.effect = calcES(data=bal.data, w=W[,iters.grid] , treat=data[,treat.var],numeric.vars = numeric.vars , estimand=estimand , multinom=multinom , sw=sampW, moderator.var = moderator.var)
    ...
    std.effect = calcES(data=bal.data, w=W[,iters.es] , treat=data[,treat.var],numeric.vars = numeric.vars , estimand=estimand , multinom=multinom , sw=sampW, moderator.var = moderator.var)
  ...
  if ( any(grepl("ks.",stop.method.names)) ){
    ...
    ks.effect = calcKS(data=bal.data,w=W[,iters.grid],treat=data[,treat.var] , multinomATE=(estimand=="ATE" && multinom) , sw=sampW, moderator.var = moderator.var)
    ...
    ks.effect = calcKS(data=bal.data,w=W[,iters.ks],treat=data[,treat.var] , multinomATE=(estimand=="ATE" && multinom) , sw=sampW, moderator.var = moderator.var)
