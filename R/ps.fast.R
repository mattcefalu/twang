...
ps.fast<-function(formula ,
   ...
   if ( n.trees/n.keep< n.grid ){
      stop('n.tress must be at least n.grid*n.keep')
   }
   ...
   if (version=="gbm"){
      ...
      gbm1 <- xgboost(data=sparse.data , label=data[,treat.var], params=params, tree_method = tree_method, 
                      nrounds=n.trees , verbose=verbose , weight = sampW, 
   ...
   if (version=="gbm"){
      ps = plogis(predict(gbm1 , newdata = data , n.trees=iters[iters.grid]))
   }else{
      ps = NULL
      for (i in iters.grid){
         ps <- cbind(ps,predict(gbm1, sparse.data , ntreelimit = iters[i]))
      }
   }
   
   # weights from 25 point grid
   if (estimand=="ATE"){
   ...
   if ( any(grepl("ks.",stop.method.names)) ){
      ...
      ks.effect = calcKS(data=bal.data,w=W,treat=data[,treat.var] , multinomATE=(estimand=="ATE" & multinom) , sw=sampW)
   ...
   # finer grid of iterations
   iters.fine <- unique(c(iters.ks , iters.es))
   if (version=="gbm"){
      ps = plogis(predict(gbm1 , newdata = data , n.trees=iters[iters.fine]))
   }else{
      for (i in iters.fine){
         ps = NULL
         ps <- cbind(ps,predict(gbm1, sparse.data , ntreelimit = iters[i]))
   ...
   for (i.tp in 1:nMethod){
      ...
      if ( grepl("ks.mean",stop.method.names[i.tp]) ){
         opt = list(minimum= which.min(apply(abs(ks.effect),1,mean, na.rm=T)) )
      ...
      if ( grepl("ks.max",stop.method.names[i.tp]) ){
         opt = list(minimum= which.min(apply(abs(ks.effect),1,max, na.rm=T)) )
      ...
      if(verbose) cat("   Optimized at",iters[iters.fine[opt$minimum]],"\n")
      if(n.trees-iters[iters.fine[opt$minimum]] < 100) warning("Optimal number of iterations is close to the specified n.trees. n.trees is likely set too small and better balance might be obtainable by setting n.trees to be larger.")
      ...
      desc[[tp]]$n.trees <- ifelse(stop.method[[i.tp]]$direct, NA, iters[iters.fine[opt$minimum]])
   }
